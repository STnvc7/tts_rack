...
class APNet2Generator(Generator):
    def __init__(
        self,
        n_mels,
        fft_size,
        ...
        super(APNet2Generator, self).__init__()
        ...
        self.asp_input_norm = nn.LayerNorm(asp_channel, eps=1e-6)
        ...
        self.asp_final_norm = nn.LayerNorm(asp_channel, eps=1e-6)
    ...
    def forward(
        ...
        logamp = self.asp_input_norm(logamp.transpose(1, 2)).transpose(1, 2)
        for m in self.asp_convnext:
            logamp = m(logamp)
        logamp = self.asp_final_norm(logamp.transpose(1, 2)).transpose(1, 2)
