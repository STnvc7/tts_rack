...
class SignalGenerator(nn.Module):
    ...
    def sinusoid(self, f0):
        ...
        """
        B, _, T = f0.size()
        vuv = F.interpolate((f0 > 0) * torch.ones_like(f0), T * self.hop_size)
        # radious = (F.interpolate(f0.to(torch.float64), T * self.hop_size) / self.sample_rate) % 1
        radious = (F.interpolate(f0, T * self.hop_size) / self.sample_rate) % 1
    ...
    def sinusoids(self, f0):
        ...
        B, _, T = f0.size()
        vuv = F.interpolate((f0 > 0) * torch.ones_like(f0), T * self.hop_size)
        # f0 = F.interpolate(f0.to(torch.float64), T * self.hop_size)
        f0 = F.interpolate(f0, T * self.hop_size)
    ...
    def vuv_binary(self, f0):
        ...
        """
        _, _, T = f0.size()
        uv = F.interpolate((f0 > 0) * torch.ones_like(f0), T * self.hop_size)
