...
from .layer import AdaptiveResidualBlock, Conv1d, ResidualBlock

from utils.tensor import to_numpy
from interface.model import GeneratorOutput, Generator
from interface.loggable import Audio, Sequence
...
class SiFiGANGenerator(Generator):
    ...
    def dilated_factor(self, cf0):
        ...
        for idx in range(B):
            ...
            for df, us in zip(self.dense_factors, self.prod_upsample_scales):
                dfs += [np.repeat(dilated_factor(cf0[idx], self.sample_rate, df), us)]
            for i in range(len(self.dense_factors)):
                dfs_batch[i] += [dfs[i].reshape(-1, 1)]
        for i in range(len(self.dense_factors)):
            dfs_batch[i] = torch.FloatTensor(np.array(dfs_batch[i])).transpose(2,1).to(device)
    ...
    def forward(
        ...
        # mcep = input_feature["mcep"]
        # bap = input_feature["bap"]
        # c = torch.cat((mcep, bap), dim=1)
        
        c = input_feature["mel_spectrogram"]
        cf0 = input_feature["continuous_pitch"]
        
        d = self.dilated_factor(cf0.squeeze(1))
        x = self.signal_generator(cf0)
        ...
        output = GeneratorOutput(
            ...
            outputs={"source": e_},
            loggable_outputs={
                "source_audio": Audio(data=e_.squeeze(), sample_rate=self.sample_rate),
                "cf0": Sequence(data=cf0.squeeze())
            }
        )
        return output
